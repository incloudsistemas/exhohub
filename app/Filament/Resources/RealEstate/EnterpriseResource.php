...
use App\Filament\Resources\RealEstate\EnterpriseResource\Pages;
use App\Filament\Resources\RealEstate\EnterpriseResource\RelationManagers;
...
class EnterpriseResource extends Resource
{
        ...
        return Forms\Components\Section::make(__('Infos. Gerais'))
            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Select::make('property.usage')
                            ...
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get): void {
                ...
                Forms\Components\Select::make('property.type_id')
                    ...
                    ->live()
                    ->afterStateUpdated(
                        function (PropertyService $service, ?string $state, callable $set, callable $get): void {
                            $set('property.subtype_id', null);

                            if (!$get('property.custom_code')) {
                                $code = $service->getPropertyCode(typeId: $state);
                ...
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Select::make('min_bedroom')
                            ->label(__('Quartos (min)'))
                            ...
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('max_bedroom')) || $state > $get('max_bedroom')) {
                                        $set('max_bedroom', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('max_bedroom')
                            ->label(__('Quartos (max)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required(
                                fn(callable $get): bool =>
                                !empty($get('min_bedroom'))
                            )
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('min_bedroom')) || $state < $get('min_bedroom')) {
                                        $set('min_bedroom', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('min_suite')
                            ->label(__('Suítes (min)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('max_suite')) || $state > $get('max_suite')) {
                                        $set('max_suite', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('max_suite')
                            ->label(__('Suítes (max)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required(
                            ...
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('min_suite')) || $state < $get('min_suite')) {
                                        $set('min_suite', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('min_bathroom')
                            ->label(__('Banheiros (min)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required()
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('max_bathroom')) || $state > $get('max_bathroom')) {
                                        $set('max_bathroom', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('max_bathroom')
                            ->label(__('Banheiros (max)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required(
                                fn(callable $get): bool =>
                                !empty($get('min_bathroom'))
                            )
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('min_bathroom')) || $state < $get('min_bathroom')) {
                                        $set('min_bathroom', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('min_garage')
                            ->label(__('Vagas garagem (min)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required()
                            ->live()
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('max_garage')) || $state > $get('max_garage')) {
                                        $set('max_garage', $state);
                                    }
                                }
                            ),
                        Forms\Components\Select::make('max_garage')
                            ->label(__('Vagas garagem (max)'))
                            ->options(array_combine(range(0, 50), range(0, 50)))
                            ->native(false)
                            ->required(
                                fn(callable $get): bool =>
                            ...
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (empty($get('min_garage')) || $state < $get('min_garage')) {
                                        $set('min_garage', $state);
                                    }
                                }
                            ),
                        Forms\Components\TextInput::make('min_useful_area')
                            ...
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (
                                        empty($get('max_useful_area')) ||
                                        ConvertPtBrFloatStringToInt($state) > ConvertPtBrFloatStringToInt($get('max_useful_area'))
                                    ) {
                        ...
                        Forms\Components\TextInput::make('max_useful_area')
                            ...
                            ->afterStateUpdated(
                                function (callable $set, callable $get, ?string $state): void {
                                    if (
                                        empty($get('min_useful_area')) ||
                                        ConvertPtBrFloatStringToInt($state) < ConvertPtBrFloatStringToInt($get('min_useful_area'))
                                    ) {
        ...
        return Forms\Components\Section::make(__('Infos. Detalhadas'))
            ...
            ->schema([
                Forms\Components\Select::make('role')
                    ->label(__('Estágio do lançamento'))
                    ->options(EnterpriseRoleEnum::class)
                    ->selectablePlaceholder(false)
                    ->default(1)
                    ->required()
                    ->native(false),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\TextInput::make('property.code')
                            ...
                            ->required()
                            ->rules([
                                function (PropertyService $service, ?Enterprise $record): Closure {
                                    return function (
                                        string $attribute,
                                        string $state,
                                        Closure $fail
                ...
                Forms\Components\TextInput::make('property.title')
                    ->label(__('Nome do empreendimento'))
                    ->required()
                    ->minLength(2)
                    ->maxLength(255)
                    ->live(debounce: 1000)
                ...
                Forms\Components\TextInput::make('min_price')
                    ...
                    ->afterStateUpdated(
                        function (callable $set, callable $get, ?string $state): void {
                            if (
                                empty($get('max_price')) ||
                                ConvertPtBrFloatStringToInt($state) > ConvertPtBrFloatStringToInt($get('max_price'))
                            ) {
                                $set('max_price', $state);
                            }
                        }
                ...
                Forms\Components\TextInput::make('max_price')
                    ...
                    ->afterStateUpdated(
                        function (callable $set, callable $get, ?string $state): void {
                            if (
                                empty($get('min_price')) ||
                                ConvertPtBrFloatStringToInt($state) < ConvertPtBrFloatStringToInt($get('min_price'))
                            ) {
                                $set('min_price', $state);
                            }
                        }
    ...
    protected static function getCaptureInfosFormSection(): Forms\Components\Section
    {
        return Forms\Components\Section::make(__('Captação'))
            ->description(__('Informe os detalhes da construtora, incorporadora e do captador do imóvel.'))
            ->schema([
                Forms\Components\Select::make('property.contact_construction_companies')
                    ->label(__('Construtora(s)'))
                    ->multiple()
                    ->searchable()
                    ->preload()
                    ->getSearchResultsUsing(
                        fn(LegalEntityService $service, string $search): array =>
                        $service->getLegalEntityOptionsBySearch(search: $search, return: 'contactId'),
                    )
                    ...
                    ->when(
                        ...
                        $component->suffixAction(
                            ...
                            $service->getQuickCreateActionByContactLegalEntities(
                                field: 'property.contact_construction_companies',
                                multiple: true,
                                return: 'contactId'
                            ),
                        ),
                    ...
                    ->columnSpanFull(),
                Forms\Components\Select::make('property.contact_development_companies')
                    ->label(__('Incorporadora(s)'))
                    ->multiple()
                    ->searchable()
                    ->preload()
                    ->getSearchResultsUsing(
                    ...
                    ->when(
                        ...
                        $component->suffixAction(
                            fn(LegalEntityService $service): Forms\Components\Actions\Action =>
                            $service->getQuickCreateActionByContactLegalEntities(
                                field: 'property.contact_development_companies',
                                multiple: true,
                                return: 'contactId'
                            ),
                        ),
        ...
        return Forms\Components\Section::make(__('Publicação'))
            ...
            ->schema([
                ...
                Forms\Components\Fieldset::make(__('Datas da postagem'))
                    ...
                    ->hidden(
                        fn(callable $get): bool =>
                        !$get('property.publish_on.canal_pro') && !$get('property.publish_on.portal_exho') && !$get('property.publish_on.portal_web')
                    ),
                ...
                Tables\Actions\ActionGroup::make([
                    Tables\Actions\ActionGroup::make([
                        Tables\Actions\ViewAction::make()
                            ->extraModalFooterActions([
                                Tables\Actions\Action::make('edit')
                                    ...
                                    ->button()
                                    ->url(
                                        fn(Enterprise $record): string =>
                                        self::getUrl('edit', ['record' => $record]),
                                    )
                                    ->hidden(
                                        fn(): bool =>
                                        !auth()->user()->can('Editar [IMB] Lançamentos')
                                    ),
                            ]),
                        ...
                        Tables\Actions\Action::make('download-images')
                            ...
                            ->action(
                                fn(PropertyService $service, Enterprise $record) =>
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->before(
                            fn(EnterpriseService $service, Tables\Actions\DeleteAction $action, Enterprise $record) =>
    ...
    protected static function getTableColumns(): array
    {
        return [
            ...
            Tables\Columns\TextColumn::make('property.title')
                ...
                ->formatStateUsing(
                    fn(PropertyService $service, Enterprise $record): string =>
            ...
            Tables\Columns\TextColumn::make('role')
                ->label(__('Estágio'))
                ->searchable(
                    query: fn(EnterpriseService $service, Builder $query, string $search): Builder =>
                    $service->tableSearchByRole(query: $query, search: $search)
                )
                ->sortable(
                    query: fn(EnterpriseService $service, Builder $query, string $direction): Builder =>
            ...
            Tables\Columns\TextColumn::make('display_price')
                ...
                ->sortable(
                    query: fn(EnterpriseService $service, Builder $query, string $direction): Builder =>
                    $service->tableSortByMinPrice(query: $query, direction: $direction)
                )
                ->toggleable(isToggledHiddenByDefault: true),
    ...
    protected static function getTableFilters(): array
    {
        return [
            Tables\Filters\SelectFilter::make('types')
                ...
                ->options(
                    fn(PropertyService $service): array =>
                    $service->getOptionsByPropertyTypesWhereHasProperties(propertableType: MorphMapByClass(static::$model)),
                )
                ->query(
            ...
            Tables\Filters\SelectFilter::make('role')
                ->label(__('Estágio(s) do(s) lançamento(s)'))
                ->options(EnterpriseRoleEnum::class)
                ->multiple(),
            Tables\Filters\SelectFilter::make('districts')
                ...
                ->options(
                    fn(PropertyService $service): array =>
                    $service->getOptionsByPropertyDistrictsWhereHasProperties(propertableType: MorphMapByClass(static::$model)),
                )
                ->query(
                    fn(PropertyService $service, Builder $query, array $data): Builder =>
            ...
            Tables\Filters\Filter::make('price')
                ...
                ->query(
                    fn(EnterpriseService $service, Builder $query, array $data): Builder =>
            ...
            Tables\Filters\SelectFilter::make('contact_construction_companies')
                ->label(__('Construtor(as)'))
                ->options(
                    fn(EnterpriseService $service): array =>
                    $service->getOptionsByPropertyContactCompaniesRoleWhereHasProperties(role: 2), // 2 - Construtoras
                )
                ->query(
                    fn(EnterpriseService $service, Builder $query, array $data): Builder =>
                    $service->tableFilterByPropertyContactCompaniesRole(query: $query, role: 2, data: $data)
                )
                ->multiple(),
            Tables\Filters\SelectFilter::make('contact_development_companies')
                ->label(__('Incorporador(as)'))
                ->options(
                    fn(EnterpriseService $service): array =>
                    $service->getOptionsByPropertyContactCompaniesRoleWhereHasProperties(role: 3), // 3 - Incorporadoras
                )
                ->query(
                    fn(EnterpriseService $service, Builder $query, array $data): Builder =>
                    $service->tableFilterByPropertyContactCompaniesRole(query: $query, role: 3, data: $data)
                )
                ->multiple(),
            Tables\Filters\SelectFilter::make('owners')
                ...
                ->options(
                    fn(PropertyService $service): array =>
                    $service->getOptionsByPropertyOwnersWhereHasProperties(propertableType: MorphMapByClass(static::$model)),
                )
                ->query(
                    fn(PropertyService $service, Builder $query, array $data): Builder =>
                    ...
                    ->tabs([
                        Infolists\Components\Tabs\Tab::make(__('Infos. Gerais'))
                            ->schema([
                                ...
                                Infolists\Components\TextEntry::make('display_bedroom')
                                    ->label(__('Quarto(s)'))
                                    ->helperText(
                                        fn(Enterprise $record): ?string =>
                                        $record->display_suite ? $record->display_suite . ' suíte(s)' : null,
                                    )
                                    ->visible(
                                        fn(?string $state): bool =>
                                        !empty($state),
                                    ),
                                Infolists\Components\TextEntry::make('display_bathroom')
                                    ->label(__('Banheiro(s)'))
                                    ->visible(
                                        fn(?string $state): bool =>
                                        !empty($state),
                                    ),
                                Infolists\Components\TextEntry::make('display_garage')
                                    ->label(__('Vaga(s)'))
                                    ->visible(
                                        fn(?string $state): bool =>
                                        !empty($state),
                        ...
                        Infolists\Components\Tabs\Tab::make(__('Dados da Captação e Privativos'))
                            ->schema([
                                Infolists\Components\RepeatableEntry::make('property.contacts')
                                    ->label('Construtora(s) / Incorporadora(s)')
                                    ->schema([
                                ...
                                Infolists\Components\Fieldset::make('Captador')
                                    ->schema([
                                        ...
                                        Infolists\Components\TextEntry::make('property.owner_notes')
                                            ...
                                            ->hidden(
                                                function (Enterprise $record): bool {
                                                    $user = auth()->user();
